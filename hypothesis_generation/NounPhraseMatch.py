...
class NounPhraseMatch(CandidateExtractor):
    """Candidates correspond to all noun phrases as extracted via TextBlob"""
    def init(self):
        self.label = self.opts['label']
        self.ignore_case = self.opts.get('ignore_case', True)

    def match_tokens(self, list1, list2, noun_phrases, text_lower_tokens,sentence_pointer,np_len):
	...
	while noun_phrases_pointer < len(noun_phrases):
	    ...
	    if not self.match_tokens(raw_tokens, np_tokens, noun_phrases, text_lower_tokens, sentence_pointer,np_len):
